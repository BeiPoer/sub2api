...
// Fork image-generation settings and site skins must survive upstream feature additions.
func TestSettingService_GetPublicSettingsForInjection_PreservesForkSettings(t *testing.T) {
	for _, enabled := range []bool{false, true} {
		repo := &settingPublicRepoStub{values: map[string]string{
			SettingKeySiteSkin:            "clear",
			SettingKeySubscriptionEnabled: "false",
		}}
		svc := NewSettingService(repo, &config.Config{ImageGeneration: config.ImageGenerationConfig{
			ShowPage:       enabled,
			GPT2KGroupIDs:  "11",
			GPT4KGroupIDs:  "22",
			GeminiGroupIDs: "33",
		...
		payload, ok := raw.(*PublicSettingsInjectionPayload)
		require.True(t, ok)
		require.False(t, payload.SubscriptionEnabled)
		require.Equal(t, "clear", payload.SiteSkin)
		require.Equal(t, enabled, payload.ShowImageGenerationPage)
		require.Equal(t, []int64{11, 22, 33, 44}, payload.ImageGenerationGroupIDs)
		require.Equal(t, []int64{11}, payload.GPT2KImageGenerationGroupIDs)
		require.Equal(t, []int64{22}, payload.GPT4KImageGenerationGroupIDs)
		require.Equal(t, []int64{33}, payload.GeminiImageGenerationGroupIDs)
	}
}
